...
urlpatterns = patterns('',
    ...
    url(r'^cloud/(?P<uuid>[\w-]+)/cluster/(?P<cluster_id>[\w-]+)/(?P<asg_def_id>[\w-]+)/deregister_asg$', 'cloud.views.auto_scaling_group_def_handle', {'action': 'deregister_asg'}),
    url(r'^cloud/(?P<uuid>[\w-]+)/cluster/(?P<cluster_id>[\w-]+)/(?P<asg_def_id>[\w-]+)/start$', 'cloud.views.auto_scaling_group_def_handle', {'action': 'start'}),
    url(r'^cloud/(?P<uuid>[\w-]+)/cluster/(?P<cluster_id>[\w-]+)/(?P<asg_def_id>[\w-]+)/stop_instances$', 'cloud.views.auto_scaling_group_def_handle', {'action': 'stop_instances'}),
    url(r'^cloud/(?P<uuid>[\w-]+)/cluster/(?P<cluster_id>[\w-]+)/add_asg_def$', 'cloud.views.auto_scaling_group_def_add'),
